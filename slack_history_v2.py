...
# Minor modification to pick only my channels; hardcoded [Cano]
#
...
def getChannels(slack, dryRun):
  ...
  if not dryRun:
    ...
    mkdir(parentDir)
    for channel in channels:
      if (channel['name'].startswith("op_analytics")) or (channel['name'].startswith("crawler")):
        ...
        print("skipping channel {0}".format(channel['name']))
